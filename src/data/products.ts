...
// NOTE: This is placeholder seed data — real product photography, and
...
//
// Images live under public/images/products/ with STABLE filenames (not
// Vite's content-hashed build output) precisely so a URL seeded into the
// database today still resolves after a future rebuild/redeploy.

function img(id: string, alt: string, variant: "" | "-b" = "") {
  return { id: `${id}${variant}`, url: `/images/products/prod-${id}${variant}.jpg`, alt };
}
...
// All seed products start active; `active` is intentionally omitted from
// each entry below and applied once in seedProducts, so this list didn't
// need every object touched when the active/inactive admin toggle was added.
const rawSeedProducts: Array<Omit<Product, "active">> = [
  {
    id: "p1",
...
export const seedProducts: Product[] = rawSeedProducts.map((p) => ({ ...p, active: true }));
