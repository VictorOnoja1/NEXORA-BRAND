  ...
  return (
    <div className="max-w-8xl mx-auto px-4 md:px-10 py-10 md:py-16">
      <div className="text-center max-w-xl mx-auto mb-12">
        <p className="text-xs font-semibold tracking-widest2 uppercase text-black mb-2">Get in Touch</p>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          {submitted ? (
            <div className="border border-plum-100 rounded-lg p-8 text-center">
              <CheckCircle2 size={32} className="text-black mx-auto mb-4" />
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div>
                <label className="text-xs font-medium text-black mb-1.5 block" htmlFor="name">Name</label>
                <input
                  id="name"
                  required
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  className="w-full border border-plum-200 rounded px-4 py-3 text-sm text-chocolate placeholder:text-black focus:outline-none focus:border-plum"
                  placeholder="Your name"
                />
              </div>
              <div>
                <label className="text-xs font-medium text-black mb-1.5 block" htmlFor="c-email">Email</label>
                <input
                  id="c-email"
                  type="email"
                  required
              ...
              <div>
                <label className="text-xs font-medium text-black mb-1.5 block" htmlFor="message">Message</label>
                <textarea
                  id="message"
                  required
                  rows={5}
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-black font-sans">Fastest way to reach our team</p>
            </div>
          </a>
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-black font-sans">{siteConfig.supportEmail}</p>
            </div>
          </a>
          ...
          <div className="flex items-center gap-4 border border-plum-100 rounded-lg p-5">
            ...
            <div>
              ...
              <p className="text-xs text-black font-sans">Shipping across Nigeria</p>
            </div>
          </div>

          <div className="flex items-center gap-3 mt-2">
            <a href={siteConfig.socials.instagram} target="_blank" rel="noreferrer" aria-label="Instagram" className="w-9 h-9 rounded-full border border-plum-200 flex items-center justify-center text-black hover:bg-plum hover:text-ivory transition-colors">
              <InstagramIcon size={15} />
            </a>
            <a href={siteConfig.socials.tiktok} target="_blank" rel="noreferrer" aria-label="TikTok" className="w-9 h-9 rounded-full border border-plum-200 flex items-center justify-center text-black hover:bg-plum hover:text-ivory transition-colors">
              <TikTokIcon size={15} />
            </a>
          </div>
        </motion.div>
          ...
          {faqs.map((f) => (
            <details key={f.q} className="group py-4">
              <summary className="flex items-center justify-between cursor-pointer text-sm font-medium text-chocolate list-none">
                {f.q}
                <span className="text-black group-open:rotate-45 transition-transform text-lg leading-none">+</span>
              </summary>
              <p className="text-sm text-black mt-2 font-sans leading-relaxed">{f.a}</p>
            </details>
          ))}
        </div>
      </div>
