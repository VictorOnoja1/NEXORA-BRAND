...
export default function Shop() {
  ...
  return (
    <div className="max-w-8xl mx-auto px-4 md:px-10 py-6 md:py-10">
      {/* Breadcrumb */}
      <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs text-black mb-4 font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center border border-plum-200 rounded overflow-hidden">
            <button
              ...
              className={`p-2.5 ${view === "grid" ? "bg-plum text-ivory" : "text-black"}`}
            >
              <LayoutGrid size={16} />
            </button>
            <button
              ...
              className={`p-2.5 ${view === "list" ? "bg-plum text-ivory" : "text-black"}`}
            >
              <ListIcon size={16} />
            </button>
          </div>
              ...
              <div className="flex-1 flex flex-col justify-between min-w-0">
                <div>
                  <Link to={`/product/${p.slug}`}>
                    <h3 className="font-serif text-lg text-chocolate mb-1 hover:text-black">{p.name}</h3>
                  </Link>
                  <p className="text-sm text-black mb-2 line-clamp-2 font-sans">{p.shortDescription}</p>
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <button
                    ...
                    disabled={p.stock === 0}
                    className="text-xs font-semibold uppercase tracking-widest2 text-black border border-plum rounded px-4 py-2 hover:bg-plum hover:text-ivory transition-colors disabled:opacity-30"
                  >
                    Add to Cart
                  </button>
                </div>
