...
export default function AdminOrders() {
  ...
  // Refresh from Supabase every time an admin visits this page — the
  // module-level load on app start (see orderStore.ts) can otherwise go
  // stale if orders come in while the dashboard is open elsewhere. No-op
    ...
    <div>
      ...
      <div className="border border-plum-100 rounded-lg overflow-hidden overflow-x-auto">
            ...
            {filtered.map((o) => (
              <tr key={o.id} className="border-b border-plum-50 last:border-0 hover:bg-blush/10">
                <td className="px-5 py-3">
                  <Link to={`/admin/orders/${o.id}`} className="text-chocolate font-medium hover:text-black">{o.orderNumber}</Link>
                </td>
        ...
        {loading && orders.length === 0 && (
          <p className="text-sm text-black font-sans px-5 py-10 text-center">Loading orders…</p>
        )}
      </div>
    </div>
  );
