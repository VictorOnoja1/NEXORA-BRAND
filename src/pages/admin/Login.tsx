...
export default function AdminLogin() {
  ...
  // Already signed in — go straight to the dashboard (or wherever the
  // visitor was headed before being bounced here).
  if (isAuthenticated) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSubmitting(false);
    if (!ok) {
      setError("Incorrect email/username or password, or this account isn't an admin.");
      return;
    }
    const from = (location.state as { from?: string } | null)?.from || "/admin";
