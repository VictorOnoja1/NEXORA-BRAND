...
function StatusRow({ label, configured, hint }: { label: string; configured: boolean; hint: string }) {
  return (
    <div className="flex items-start gap-3 py-3 border-b border-plum-50 last:border-0">
      {configured ? (
        <CheckCircle2 size={17} className="text-black shrink-0 mt-0.5" />
      ) : (
      ...
      <div>
        ...
        <p className="text-xs text-black font-sans mt-0.5">{hint}</p>
      </div>
    </div>
  );
}
...
export default function AdminSettings() {
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-sm text-black mb-8 font-sans">Store configuration and backend connections.</p>
      ...
      <div className="border border-plum-100 rounded-lg p-5 mb-6">
        ...
        <dl className="flex flex-col gap-3 text-sm font-sans">
          <div className="flex justify-between"><dt className="text-black">WhatsApp Number</dt><dd className="text-chocolate">{siteConfig.whatsappNumber}</dd></div>
          <div className="flex justify-between"><dt className="text-black">Support Email</dt><dd className="text-chocolate">{siteConfig.supportEmail}</dd></div>
          <div className="flex justify-between"><dt className="text-black">Flat Delivery Fee</dt><dd className="text-chocolate">₦{siteConfig.flatDeliveryFee.toLocaleString()}</dd></div>
          <div className="flex justify-between"><dt className="text-black">Free Delivery Threshold</dt><dd className="text-chocolate">₦{siteConfig.freeDeliveryThreshold.toLocaleString()}</dd></div>
          <div className="flex justify-between"><dt className="text-black">Announcement Bar</dt><dd className="text-chocolate text-right max-w-[60%]">{siteConfig.announcement}</dd></div>
        </dl>
        <p className="text-xs text-black font-sans mt-4">
